...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		{
            /* Editing the movement to also work with keybord and joystick controls*/
            ...
            //if we shouldn't move, then dont
            if (!canMove)
            {
                moveInfluence = 0;
            }
            else if ( cam.CameraUp == true || (Input.GetAxisRaw("Creep") > 0 || Input.GetAxisRaw("Oculus_GearVR_RIndexTrigger") > 0)) // IF taking a picture or holding the trigger then creep
            {
                isCreeping = true;
                moveInfluence /= 2;
            }
